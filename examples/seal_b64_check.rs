...
fn main() {
    ...
    let resp_pub_b58 = args
        .next()
        .expect("usage: seal_b64_check <resp_pub_b58> <plaintext>");
    let plaintext = args
        .next()
        .expect("usage: seal_b64_check <resp_pub_b58> <plaintext>");

    let resp_pub_vec = bs58::decode(&resp_pub_b58)
        .into_vec()
        .expect("bad resp pub b58");
    let resp_pub: [u8; 32] = resp_pub_vec
        .as_slice()
        .try_into()
        .expect("resp pub must be 32 bytes");
